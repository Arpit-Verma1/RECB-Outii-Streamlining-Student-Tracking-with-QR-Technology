...
class _qrscanState extends State<qrscan> {
  ...
  String Year="";
  int year=int.parse(DateFormat("yyyy").format(DateTime.now()).substring(2,4));
  String Branch="";
  String Name="";
  int k=0;
  String Rollno="";
...
TextEditingController Controller=TextEditingController();
  ...
  Widget buildresult()=>Container(margin: EdgeInsets.all(5),
      ...
      width: MediaQuery.of(context).size.width,
      height: MediaQuery.of(context).size.height*0.12,child:Column(children: [
        Row(children:[Text("Name:- ",style: TextStyle(fontWeight: FontWeight.bold),)
          ,Text(barcode==null?'--':'${barcode!.code}'.substring(0,'${barcode!.code}'.indexOf(".")).toUpperCase()),
          SizedBox(width: MediaQuery.of(context).size.width*0.05,),
          Text("Branch:- ",style: TextStyle(fontWeight: FontWeight.bold)),
          Text(barcode==null?'--':'${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+1,'${barcode!.code}'.indexOf(".")+3).toUpperCase()),
          SizedBox(width: MediaQuery.of(context).size.width*0.05,),
          Text('Phone:- ',style: TextStyle(fontWeight: FontWeight.bold)),
          Text(barcode==null?'--':'${barcode!.code}'.substring('${barcode!.code}'.indexOf("@")+11,'${barcode!.code}'.indexOf("@")+21)),

        ]),
        Row(children: [

          Text("Roll No:- ",style: TextStyle(fontWeight: FontWeight.bold)),
          Text(barcode==null?'--':'${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+7,'${barcode!.code}'.indexOf(".")+9)),
          SizedBox(width: MediaQuery.of(context).size.width*0.05,),
          Text("Year:-",style: TextStyle(fontWeight: FontWeight.bold)),
          Text(barcode==null?'--':(
              int.parse('${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+4,'${barcode!.code}'.indexOf(".")+6))==year?"1":(
                  int.parse('${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+4,'${barcode!.code}'.indexOf(".")+6))+1==year?"2":(
                      int.parse('${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+4,'${barcode!.code}'.indexOf(".")+6))+2==year?"3":"4"
                  )
              ))
          ),
          SizedBox(width: MediaQuery.of(context).size.width*0.05,),
          Text("Purpose:- ",style: TextStyle(fontWeight: FontWeight.bold)),
          Text(barcode==null?'--':'${barcode!.code}'.substring('${barcode!.code}'.indexOf("@")+21,))
        ],),
...
SizedBox(height: MediaQuery.of(context).size.height*0.01,),
       Row(mainAxisAlignment: MainAxisAlignment.center,
         ...
         _createdb();
       }, child: Text("Check In")),SizedBox(width: MediaQuery.of(context).size.width*0.07,)
           ,ElevatedButton(onPressed: (){
         _createdb1();
       }, child: Text("Check Out")),SizedBox(width: MediaQuery.of(context).size.width*0.07,),
Container(padding: EdgeInsets.only(left: 5),
  height:MediaQuery.of(context).size.height*0.05,
  decoration: BoxDecoration(color: Colors.blue,borderRadius: BorderRadius.circular(5)),
  child: Row(
    children: [ Text("Parents",style: TextStyle(color: Colors.white,fontWeight: FontWeight.bold),),
  Switch(activeTrackColor: Colors.black,
      activeColor: Colors.white,
      value: sendDirect, onChanged: (bool newValue) {
    text();

    setState(() {
      email='${barcode!.code}'.substring(0,'${barcode!.code}'.indexOf(".")+20);
  ...
  _createdb()async{
    ...
    setState(() {
      ...
      f=e+int.parse('${barcode!.code}'[d+7]);
       ...
       message1="Your child $Name is going to $Where at ${Datein()} -regards hostel warden";
    });
    people.add("$phone");
    if(sendDirect==true){
    people1.add(email);
  ...
  Future sendEmail()async{
   final _googlesignin=GoogleSignIn(scopes: ['https://mail.google.com/']);
    ...
    final message=Message()
      ..from=Address(email)
      ..recipients=barcode==null?people1:(
          int.parse('${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+4,'${barcode!.code}'.indexOf(".")+6))==year?people1:(
              int.parse('${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+4,'${barcode!.code}'.indexOf(".")+6))+1==year?people2:(
                  int.parse('${barcode!.code}'.substring('${barcode!.code}'.indexOf(".")+4,'${barcode!.code}'.indexOf(".")+6))+2==year?people3:people4
              )
          ))
